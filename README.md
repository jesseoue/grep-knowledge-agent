...
Build an AI agent that stays up to date with your docs, codebases, and transcripts by giving the LLM a filesystem and `bash`. It runs `grep`, `find`, and `cat` against a snapshot of your sources — results are **deterministic, explainable, and ~75% cheaper** than vector RAG. Forked from the viral [vercel-labs/knowledge-agent-template](https://github.com/vercel-labs/knowledge-agent-template).
...
1. **Set an AI provider key** — at least one of:
   ...
   - `GOOGLE_GENERATIVE_AI_API_KEY` → get one at <https://aistudio.google.com/apikey>
